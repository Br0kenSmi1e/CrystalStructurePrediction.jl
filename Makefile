...
init:
	$(JL) -e 'using Pkg; Pkg.precompile(); Pkg.activate("docs"); Pkg.develop(path="."); Pkg.instantiate(); Pkg.activate("examples"); Pkg.develop(path="."); Pkg.instantiate()'

run-example:
	$(JL) -e 'using Pkg; Pkg.activate("examples"); include("examples/SrTiO3.jl")'
...
.PHONY: init test coverage serve clean update run-example